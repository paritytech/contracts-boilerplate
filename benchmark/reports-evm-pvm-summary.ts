...
function baseVsMeteredTable(): string {
	const rows = db.prepare(`
		...
		GROUP BY tx_type
		ORDER BY tx_type
	`).all() as { tx_type: string; avg_base_rt: number; avg_metered_rt: number; base_rt_pct: number; metered_rt_pct: number; avg_base_pov: number; avg_metered_pov: number; base_pov_pct: number | null; metered_pov_pct: number | null }[]
...
function topOpsTable(deploy: boolean): string {
	...
	return table([
		...top3.map(r => makeRow(r.category, r.total_rt, r.total_pov, fmt(r.calls))),
...
function execTotalsTable(): string {
	...
	let md = `**${allRow.n} EVM↔PVM/Solidity pairs (excluding CoinTool_App):**\n`
	md += table([
		{ 'Metric': 'Metered ref_time', 'EVM': fmt(Math.round(allRow.evm_rt)), 'PVM/Solidity': fmt(Math.round(allRow.pvm_rt)), 'Diff': fmtPct(allRow.pvm_rt, allRow.evm_rt) },
		{ 'Metric': 'Metered proof_size', 'EVM': fmt(Math.round(allRow.evm_pov)), 'PVM/Solidity': fmt(Math.round(allRow.pvm_pov)), 'Diff': fmtPct(allRow.pvm_pov, allRow.evm_pov) },
	]) + '\n\n'
	...
	const makeRow7 = (metric: string, evm: number, sol: number, rust: number) => ({
		'Metric': metric,
	...
	md += table([
		...
		makeRow7('Metered proof_size', row7.evm_pov, row7.pvm_pov, row7.rust_pov),
	]) + '\n\n'
	...
	md += table([
		{
			...
			'Txs cheaper': `${solRts.filter(p => p < 0).length}/${solRts.length}`,
			'Median proof_size': pct(median(solPovs)),
			'Txs cheaper ': `${solPovs.filter(p => p < 0).length}/${solPovs.length}`,
		},
		{
			'Comparison': `PVM/Rust vs EVM (${rustRts.length} txs)`,
			'Median ref_time': pct(median(rustRts)),
			'Txs cheaper': `${rustRts.filter(p => p < 0).length}/${rustRts.length}`,
			'Median proof_size': pct(median(rustPovs)),
			'Txs cheaper ': `${rustPovs.filter(p => p < 0).length}/${rustPovs.length}`,
		},
		{
			'Comparison': `PVM/Rust vs PVM/Sol (${rvsRts.length} txs)`,
			'Median ref_time': pct(median(rvsRts)),
			'Txs cheaper': `${rvsRts.filter(p => p < 0).length}/${rvsRts.length}`,
			'Median proof_size': rvsPovs.length > 0 ? pct(median(rvsPovs)) : '—',
			'Txs cheaper ': rvsPovs.length > 0 ? `${rvsPovs.filter(p => p < 0).length}/${rvsPovs.length}` : '—',
		},
	]) + '\n\n'
...
function deployTotalsTable(): string {
	...
	md += table([
		makeRow('Base ref_time', sumField(solPairs, r => r.base_rt), sumPvm(solPairs, r => r.base_rt)),
		makeRow('Metered ref_time', sumField(solPairs, r => r.metered_rt), sumPvm(solPairs, r => r.metered_rt)),
	]) + '\n\n'
	...
	if (rustPairs.length > 0) {
		...
		md += table([
			makeRow3('Base ref_time',
				sumField(rustPairs, r => r.base_rt), sumPvm(rustPairs, r => r.base_rt), sumRust(rustPairs, r => r.base_rt)),
			makeRow3('Metered ref_time',
				sumField(rustPairs, r => r.metered_rt), sumPvm(rustPairs, r => r.metered_rt), sumRust(rustPairs, r => r.metered_rt)),
			makeRow3('Metered proof_size',
				sumField(rustPairs, r => r.metered_pov), sumPvm(rustPairs, r => r.metered_pov), sumRust(rustPairs, r => r.metered_pov)),
		]) + '\n\n'
	}
...
function bytecodeSizeTable(): string {
	...
	return table(rows.map(r => ({
		...
		'Ratio ': r.rustSize != null ? `${(r.rustSize / r.evmSize).toFixed(1)}x` : '—',
		'ink! bytes': r.inkSize != null ? fmt(r.inkSize) : '—',
		'Ratio  ': r.inkSize != null ? `${(r.inkSize / r.evmSize).toFixed(1)}x` : '—',
	}))) + '\n\n'
}
...
function simpleTokenTable(): string {
	...
	`).all() as { contract_name: string; transaction_name: string; metered: number }[]

	const txNames = [...new Set(tokens.map(r => r.transaction_name))]
	const contracts = [...new Set(tokens.map(r => r.contract_name))]

	const map = new Map<string, number>()
	for (const r of tokens) map.set(`${r.contract_name}|${r.transaction_name}`, r.metered)
	...
	const rows = contracts.map(c => {
		...
		for (const tx of txNames) {
			...
			if (val != null && evm != null) {
				const diff = c === 'SimpleToken_evm' ? '' : ` (${pct((val - evm) / evm * 100)})`
				row[tx] = fmt(val) + diff
			} else {
				row[tx] = '—'
			}
...
function fibonacciTable(): string {
	...
	const evmMetered = fib.find(r => r.contract_name === 'Fibonacci_evm')?.metered
	if (!evmMetered) return ''

	return table(fib.map(r => ({
		'Contract': r.contract_name,
		'Metered ref_time': fmt(r.metered),
		'vs EVM': r.contract_name === 'Fibonacci_evm' ? '—' : pct((r.metered - evmMetered) / evmMetered * 100),
	}))) + '\n\n'
}
...
function syscallCostTable(): string {
	...
	const rows = ops.map(op => {
		...
		return {
			...
			'EVM vs Rust': rust ? pct((rust.avg - evm.avg) / evm.avg * 100) : '—',
		}
	}).filter(r => r != null)
...
function costDecomposition7Table(): string {
	...
	const rows = [
		...mainCats.map(cat => ({
			'Category': cat,
			'EVM total (calls)': fmtTotal(evmMap, cat), 'EVM avg': fmtAvg(evmMap, cat),
			'PVM/Sol total (calls)': fmtTotal(pvmMap, cat), 'PVM/Sol avg': fmtAvg(pvmMap, cat),
			'Rust total (calls)': fmtTotal(rustMap, cat), 'Rust avg': fmtAvg(rustMap, cat),
		})),
		{
			'Category': 'Immutable data',
			'EVM total (calls)': '—', 'EVM avg': '—',
			'PVM/Sol total (calls)': fmtTotal(pvmMap, 'Immutable data'), 'PVM/Sol avg': fmtAvg(pvmMap, 'Immutable data'),
			'Rust total (calls)': '—', 'Rust avg': '—',
		},
		{
			'Category': 'Other syscalls',
			'EVM total (calls)': '—', 'EVM avg': '—',
			'PVM/Sol total (calls)': '—', 'PVM/Sol avg': '—',
			'Rust total (calls)': fmtTotal(rustMap, 'Other'), 'Rust avg': fmtAvg(rustMap, 'Other'),
		},
		{
			'Category': 'EVM opcodes',
			'EVM total (calls)': fmtTotal(evmMap, 'Other'), 'EVM avg': fmtAvg(evmMap, 'Other'),
			'PVM/Sol total (calls)': '—', 'PVM/Sol avg': '—',
			'Rust total (calls)': '—', 'Rust avg': '—',
		},
		{
			'Category': 'Unattributed',
			'EVM total (calls)': `${fmt(Math.round((evmTotals.total_rt - evmAttrRt) / 1_000_000))}M / ${fmt(Math.round((evmTotals.total_pov - evmAttrPov) / 1_000))}K`,
			'EVM avg': '',
			'PVM/Sol total (calls)': `${fmt(Math.round((pvmTotals.total_rt - pvmAttrRt) / 1_000_000))}M / ${fmt(Math.round((pvmTotals.total_pov - pvmAttrPov) / 1_000))}K`,
			'PVM/Sol avg': '',
			'Rust total (calls)': `${fmt(Math.round((rustTotals.total_rt - rustAttrRt) / 1_000_000))}M / ${fmt(Math.round((rustTotals.total_pov - rustAttrPov) / 1_000))}K`,
			'Rust avg': '',
		},
		{
			'Category': '**Total metered**',
			'EVM total (calls)': `**${fmt(Math.round(evmTotals.total_rt / 1_000_000))}M / ${fmt(Math.round(evmTotals.total_pov / 1_000))}K**`,
			'EVM avg': '',
			'PVM/Sol total (calls)': `**${fmt(Math.round(pvmTotals.total_rt / 1_000_000))}M / ${fmt(Math.round(pvmTotals.total_pov / 1_000))}K**`,
			'PVM/Sol avg': '',
			'Rust total (calls)': `**${fmt(Math.round(rustTotals.total_rt / 1_000_000))}M / ${fmt(Math.round(rustTotals.total_pov / 1_000))}K**`,
			'Rust avg': '',
		},
	]
...
function costGapDecompositionTable(): string {
	...
	md += table([
		{ 'Metric': 'Metered ref_time', 'EVM total': fmt(Math.round(raw.sum_evm_rt)), 'PVM total': fmt(Math.round(raw.sum_pvm_rt)), 'PVM - EVM': fmtDelta(rtGap), 'Diff': pct(rtGap / raw.sum_evm_rt * 100) },
		{ 'Metric': 'Metered proof_size', 'EVM total': fmt(Math.round(raw.sum_evm_pov)), 'PVM total': fmt(Math.round(raw.sum_pvm_pov)), 'PVM - EVM': fmtDelta(povGap), 'Diff': pct(povGap / raw.sum_evm_pov * 100) },
	]) + '\n\n'

	const sources = [
		{ source: 'Unattributed (interpreter + bytecode)', rt: raw.unattr_rt, pov: raw.unattr_pov },
		{ source: 'Cross-contract calls', rt: raw.call_rt, pov: raw.call_pov },
		{ source: 'Immutable data (PVM-only)', rt: raw.immut_rt, pov: raw.immut_pov },
	...
	md += table(sources.map(r => ({
		...
		'% of gap': r.source === '**Net PVM surplus**' ? '100%' : fmtPctOfGap(r.rt, rtGap),
		'proof_size (bytes)': fmtDelta(r.pov),
		'% of gap ': r.source === '**Net PVM surplus**' ? '100%' : fmtPctOfGap(r.pov, povGap),
	}))) + '\n\n'
